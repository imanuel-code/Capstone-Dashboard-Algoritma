...
def index():
    ...
    # revenue adalah dataframe yang menampilkan total jumlah aplikasi yang diinstall dan juga total sales untuk tiap kategori content rating
    # Bar plot digunakan untuk menampilkan total sales aplikasi (jumlah install * price) untuk tiap kategori content rating yang digunakan
    # untuk melihat potential market segment
    ...
    revenue = df2[['Content Rating', 'Installs', 'Revenue']].groupby(['Content Rating']).agg({'Installs' : 'sum', 'Revenue': 'sum'}).sort_values(by="Revenue", ascending=False).round(0)
    
    label = revenue.reset_index()['Content Rating']
    data = revenue.reset_index()['Revenue'] / 1000000
    fig = plt.figure(figsize=(10,5))
    fig.add_subplot()
    plt.bar(label,data,color=my_colors)
    plt.xlabel('Content Rating')
    plt.ylabel('Total Sales [in mil. USD]')
